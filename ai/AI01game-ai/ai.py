...
def minimax_alpha_beta(board, depth, alpha, beta, max_player, game, eval_params=None):
    """
        ...
        """
    # default eval_params
    eval_params = eval_params or (1.0, 1.0, 0.0, 0.0, 0.0)


    # when reached depth limitation or game has a winner, return score 
    if depth == 0 or game.winner():
        return evaluate(board, game, *eval_params), board
    
    # Player 1's best score and move
    def max_value(board, alpha, beta, depth):
        if depth == 0 or game.winner():
            return evaluate(board, game, *eval_params), None 
        
        # Initialize best score and move
        best_score = -math.inf
        best_move = None
    
        # Iterate player 2's possible moves to mind the max for player 1
        for new_board in game.generate_all_moves(board, PLAYER2_PIECE_COLOR):
            temp_board = deepcopy(new_board)   
            # player 2's best score
            score, _ = min_value(temp_board, alpha, beta, depth - 1)

            # Keep comparing 2's scores and save the best in best_score
            if score > best_score:
                best_score, best_move = score, temp_board

            # update 1's ground best
            alpha = max(alpha, best_score)

            # when 2's ground better (smaller) than 1's best_score, cut it
            if best_score >= beta:
                break
    ...
    def min_value(board, alpha, beta, depth):
        ...
        for new_board in game.generate_all_moves(board, PLAYER1_PIECE_COLOR):
            temp_board = deepcopy(new_board)  
            score, _ = max_value(temp_board, alpha, beta, depth - 1)
...
def evaluate(board, game, pieces_weight=1.0, kings_weight=1.0, moves_weight=0.0, opportunities_weight=0.0, king_hopefuls_weight=0.0):
    ...
    Notes:
        ...
        number of such pieces but as how many such king promotions can occur. 
        So, if a piece can become king in its
...
def counts(board, game, color):
    ...
    num_kings = sum(1 for piece in all_pieces if piece.king)

    num_moves = len(set(
    (piece.row, piece.col, move[0], tuple(move[1] if isinstance(move[1], list) else [move[1]]))
    for piece in all_pieces
    for move, _, _ in game.find_moves(board, piece)
    ))

    num_opportunities = len(set(
    (piece.row, piece.col, move[0], tuple(move[1] if isinstance(move[1], list) else [move[1]]))
    for piece in all_pieces
    for move, captured, _ in game.find_moves(board, piece)
    if captured and isinstance(captured, list) and len(captured) > 0 
    ))

    num_king_hopefuls = len(set(
    (piece.row, piece.col, move[0], tuple(move[1] if isinstance(move[1], list) else [move[1]]))
    for piece in all_pieces
    for move, _, king_hopeful in game.find_moves(board, piece)
    if king_hopeful 
    ))
