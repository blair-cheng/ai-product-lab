    ...
    def fit(self, env, steps=1000, num_bins=100):
        ...
        for step in range(steps):
          if src.random.rand() < self.epsilon:
            action = src.random.choice(np.arange(n_actions))
          else: 
            max_Q_action = np.where(state_action_values[current_state, :] == np.max(state_action_values[current_state,:]))[0]
            action = src.random.choice(max_Q_action)
          next_state, reward, terminated, truncated, _ = env.step(action)
          
          all_rewards.append(reward)
          state_action_values[current_state, action] = (1-self.alpha) * state_action_values[current_state, action] + self.alpha * (reward + self.gamma * np.max(state_action_values[next_state,:]) )
        ...
        for i in range(num_bins): 
          start = i * s
          end = min((i + 1)*s, len(all_rewards))
          if start < len(all_rewards):
