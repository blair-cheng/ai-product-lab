...
class MultiArmedBandit:
    ...
    def fit(self, env, steps=1000, num_bins=100):
        ...
        for step in range(steps): 
            ...
            to_explore = src.random.rand()  < self.epsilon
            if to_explore:
              action = src.random.choice(np.arange(n_actions))
            else: 
               max_Q_action = np.where(self.Q == np.max(self.Q))[0]
    ...
    def predict(self, env, state_action_values):
        ...
        # a loop to predict over each step in an episode
        actions = []
        states = []
        rewards = []
        
        while True:
          max_Q_action = np.where(state_action_values[state] == np.max(state_action_values[state]))[0]
          action = src.random.choice(max_Q_action)
