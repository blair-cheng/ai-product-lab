    ...
    def predict(self, features):
        ...
        Returns:
            labels {np.ndarray} -- Labels for each data point, of shape
                (n_samples, 1).
        """
        # distances between train and test data
        distances =  self.distance(features, self.features)

        # sort distance accending and get the front n's indecis
        nearest_neighbors = np.argsort(distances, axis = 1)[:,:self.n_neighbors]
        # nearest n neighbor's lables
        nearest_labels = self.targets[nearest_neighbors]

        # final labels
        labels = np.apply_along_axis(self.aggregator, axis = 1, arr = nearest_labels)
